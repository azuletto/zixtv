...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!upstreamResponse.ok) {
      return res.status(upstreamResponse.status).send(body || `Falha ao buscar recurso remoto (${upstreamResponse.status})`);
    }

    res.setHeader('Content-Type', upstreamResponse.headers.get('content-type') || 'text/plain; charset=utf-8');
    res.status(200).send(body);
  } catch (error) {
    res.status(500).json({ error: `Erro no proxy remoto: ${error.message}` });
  }
};